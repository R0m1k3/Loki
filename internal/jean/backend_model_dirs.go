...
func modelDirsPath() string { return filepath.Join(JeanHome(), "model_dirs.json") }
...
func resolveModelPath(name string) (string, error) {
	...
	return filepath.Join(JeanHome(), base), nil // introuvable : l'appelant décidera
}
...
func handleModelDirs(w http.ResponseWriter, r *http.Request) {
	...
	out := []map[string]any{{"path": JeanHome(), "home": true, "count": modelDirCount(JeanHome()), "exists": true}}
