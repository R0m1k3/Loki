...
func cmdInstall(args []string) error {
	if os.Geteuid() != 0 {
		return fmt.Errorf("loki install doit être exécuté en root (sudo loki install)")
	}
	...
	lokiHome := defaultLokiHome()
	if v := os.Getenv("LOKI_HOME"); v != "" {
		lokiHome = v
	}

	fmt.Printf("Installation pour utilisateur %s\n", cyan(targetUser))
	fmt.Printf("  LOKI_HOME = %s\n", lokiHome)
	...
	if err := migrateFrom07(lokiHome); err != nil {
	...
	// 2. Lien vers /usr/local/bin/loki.
	if err := linkInstalledExe(); err != nil {
		return err
	}

	// 3. /etc/default/loki, pour que les invocations root résolvent LOKI_HOME.
	if err := os.MkdirAll("/etc/default", 0o755); err != nil {
		return err
	}
	defaults := fmt.Sprintf("# Généré par loki install — racine des données\nLOKI_HOME=%s\n", lokiHome)
	if err := os.WriteFile("/etc/default/loki", []byte(defaults), 0o644); err != nil {
		return err
	}
	fmt.Printf("  %s /etc/default/loki\n", green("✓"))

	// 4. Déclaration des services (systemd ou launchd selon la plateforme).
	if err := installServices(targetUser, lokiHome); err != nil {
		return err
	}

	// 5. LOKI_HOME appartient à l'utilisateur cible : les deux services et les
...
func cmdUninstall(args []string) error {
	if os.Geteuid() != 0 {
		return fmt.Errorf("loki uninstall doit être exécuté en root")
	}
	uninstallServices()
	for _, p := range []string{"/etc/default/loki", installedExePath()} {
		if err := os.Remove(p); err == nil {
			fmt.Printf("  %s %s\n", green("✓"), p)
		}
	}
	fmt.Println(dim("(données utilisateur conservées — supprime $LOKI_HOME manuellement si tu veux purger)"))
