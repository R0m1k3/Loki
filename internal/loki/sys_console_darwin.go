...
// publie un bundle Loki.app dont le binaire vit dans
// Loki.app/Contents/MacOS/loki. Si on tourne depuis là, c'est un double-clic
// dans le Finder → expérience « application » (UI web + navigateur), exactement
// comme le double-clic sur loki.exe sous Windows.
//
...
// appWarning signale une situation de lancement qui casse Loki de façon peu
...
// Le remède est côté utilisateur : déplacer Loki.app dans /Applications.
func appWarning() string {
	...
	return "Loki tourne depuis une copie temporaire en lecture seule (App Translocation de macOS). " +
		"Fermez l'app, déplacez Loki.app dans le dossier Applications, puis rouvrez-la — " +
...
// Homebrew : sans ça, Loki ne voit ni `brew`, ni `cmake`, ni `git`, et la
