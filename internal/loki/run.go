// loki — single-binary LLM server manager + web UI for llama.cpp deployments.
// Point d'entrée réel : Main(), appelé par cmd/loki (les métadonnées Windows
...
func Main() {
	...
	// `loki` sans argument garde son comportement d'aide.
...
func printHelp() {
	fmt.Printf(`loki %s — manager llama.cpp + UI web (single binary)

Usage: loki <commande> [args]

Loki tourne en deux services : loki-engine (le modèle) et loki-ui
...
Poste distant (piloter CE PC depuis l'IA d'un serveur Loki) :
...
Env:
  LOKI_HOME   racine des données (défaut : /etc/loki, %%ProgramData%%\loki sous Windows)
  EDITOR       éditeur pour 'loki edit' (défaut : nano, notepad sous Windows)
`, Version)
}
...
func LokiHome() string {
	if h := os.Getenv("LOKI_HOME"); h != "" {
		return h
	}
	if h := readEtcDefault(); h != "" {
		return h
	}
	return defaultLokiHome()
}

// readEtcDefault parses /etc/default/loki for LOKI_HOME=<path>. Quiet on errors.
func readEtcDefault() string {
	b, err := os.ReadFile("/etc/default/loki")
	if err != nil {
		return ""
	}
	for _, line := range strings.Split(string(b), "\n") {
		...
		if k, v, ok := strings.Cut(s, "="); ok && strings.TrimSpace(k) == "LOKI_HOME" {
