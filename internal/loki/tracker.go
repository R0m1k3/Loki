...
// tracker.go — les TRACKERS : le 3ᵉ type de mémoire, pour les données DATÉES qui
// s'accumulent (compteurs, relevés, journaux d'événements). Séparé des pages .md
// (faits + procédures) exprès : un tracker grandit sans fin, donc on ne le lit
// JAMAIS en entier. On le consulte par NIVEAUX — vue d'ensemble → année → mois →
// événements — chaque réponse indiquant comment descendre. Le contexte n'explose
// donc jamais, quelle que soit l'ancienneté des données.
//
// Un seul outil `tracker` côté modèle (action view/add/edit/delete). Stockage
// structuré dans le bucket bkTracker, cloisonné par projet : clé
// « <slug-projet>/<slug-tracker> ».
...
// TrackerEvent = un point daté d'un tracker. TS = l'instant de l'événement (ms),
// Text = la valeur ou la note (« abonnés : 4210 », « pesée 78,2 kg »…).
type TrackerEvent struct {
	ID   string `json:"id"`
	TS   int64  `json:"ts"`
	Text string `json:"text"`
	// DateOnly = le point n'a PAS d'heure (juste une date). Sans ça, un point saisi
	// sans heure s'enregistre à 00:00 et s'affiche « minuit », ce qui est faux : il
	// n'a simplement pas d'heure. On ne montre alors que la date.
	DateOnly bool `json:"date_only,omitempty"`
}

// Tracker = un flux d'événements sous un nom (« abonnés », « poids »…). Events
// triés par TS croissant.
type Tracker struct {
...
// trackerStoreKey : clé de stockage d’un tracker. « Store » dans le nom parce que
// code_tracker.go porte un tout autre « tracker » (suivi lu-avant-écrit du mode
// code) et que les deux vivent dans le même paquet.
func trackerStoreKey(proj, slug string) string { return proj + "/" + slug }
...
// Sert à la suppression d'un projet (projects.go), qui emporte ses trackers.
func trackerKeysForProject(proj string) []string {
	prefix := proj + "/"
	var out []string
...
// trackerMeta = ligne légère de la vue d'ensemble (aucun événement chargé).
...
// trackerList renvoie les trackers du projet actif (métadonnées seulement), le
// plus récemment alimenté d'abord.
...
// trackerAdd ajoute un point à un tracker (créé s'il n'existe pas). `when` vide =
// maintenant.
...
// trackerEditEvent modifie un événement (texte et/ou date). when/text vides =
// inchangés.
...
func trackerDelete(slug string) error {
	return putBytes(bkTracker, trackerStoreKey(activeProjectSlug(), slug), nil)
}

// trackerRename change le NOM d'un tracker du projet actif. Le slug étant dérivé
// du nom ET servant de clé de stockage comme d'identité pour l'ajout de points
// (trackerAdd re-slugifie le nom), on re-clé le blob quand le slug change : sans
// ça, un point ajouté après renommage créerait un tracker parallèle.
...
// trackerMoveToProject déplace un tracker du projet actif vers un autre projet.
// Les octets sont déplacés tels quels : seule la clé change.
func trackerMoveToProject(slug, toSlug string) error {
	...
	fromKey, toKey := trackerStoreKey(from, slug), trackerStoreKey(toSlug, slug)
...
// dateOnly=true quand aucune heure n'a été fournie → le point n'aura pas d'heure
// d'affichage, plutôt qu'un faux « minuit ».
...
// whenParts compte les composantes datées d'un `when` (0 = rien, 1 = année,
// 2 = mois, 3 = jour, 4+ = heure). Sert à décider le NIVEAU de la vue.
...
// trackerView construit la réponse texte de l'outil `tracker` en lecture. name
// vide = vue d'ensemble ; sinon on descend selon `when` (année → mois →
// événements). Chaque réponse dit comment zoomer : le modèle sait qu'il y a un
// avant et un dessous sans jamais tout charger.
func trackerView(name, when string) string {
	...
	s, ok := trackerLoad(slug)
	if !ok {
		return fmt.Sprintf("[aucun tracker « %s »] Appelle tracker() sans argument pour la liste, ou ajoute un point avec action:\"add\".", name)
	}
	if len(s.Events) == 0 {
...
// trackerSkeleton : les années présentes (avec compte) + l'année en cours
// détaillée par mois. C'est la vue d'entrée d'un tracker.
...
// trackerGroup regroupe des événements par un format de date avec leurs comptes.
...
// trackerEventsCap plafonne le nombre d'événements listés d'un coup : au-delà,
// c'est le contexte qu'on gonflerait pour rien — un `when` plus précis affine.
const trackerEventsCap = 60

// trackerEvents liste les événements (id, date, texte), plafonné.
func trackerEvents(name, when string, ev []TrackerEvent) string {
	var b strings.Builder
	shown := ev
	trimmed := false
	if len(ev) > trackerEventsCap {
		shown = ev[len(ev)-trackerEventsCap:] // les plus récents
		trimmed = true
	}
...
// trackerIndexPrefix marque le message d'index des trackers injecté dans
// l'historique (détection / anti-doublon), comme l'index mémoire.
const trackerIndexPrefix = "Trackers (tracker tool)"

// trackerIndexMessage construit le message système listant les trackers du projet
// avec LEUR DERNIER POINT, injecté UNE FOIS en début de conversation (et après un
// compactage). L'IA sait alors qu'ils existent ET a souvent la réponse directe —
...
// trackerTool : schéma annoncé au modèle. Un seul outil pour les quatre actions —
// quatre schémas coûteraient quatre fois le contexte pour la même chose.
func trackerTool() Tool {
	return Tool{
		Type: "function",
		Function: ToolFunction{
			Name:        "tracker",
			Description: "Dated data piling up over time (counters, readings, logs). View drills down level by level: no name lists trackers, name gives years/months, name+when goes deeper.",
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"action": map[string]any{"type": "string", "enum": []string{"view", "add", "edit", "delete"}, "description": "Default \"view\"."},
					"name":   map[string]any{"type": "string", "description": "Tracker name."},
					"when":   map[string]any{"type": "string", "description": "Date, as precise as needed: \"2026\", \"2026-07\", \"2026-07-15\", \"2026-07-15 14:30\"."},
					"text":   map[string]any{"type": "string", "description": "Value or note (add/edit)."},
					"id":     map[string]any{"type": "string", "description": "Point id, shown in brackets (edit/delete)."},
				},
			},
		},
	}
...
// toolTracker exécute l'outil `tracker` et rend le texte vu par le modèle.
func toolTracker(args map[string]any) string {
	action := strings.ToLower(strings.TrimSpace(str(args["action"])))
	name := strings.TrimSpace(str(args["name"]))
	when := strings.TrimSpace(str(args["when"]))
	text := strings.TrimSpace(str(args["text"]))
	id := strings.TrimSpace(str(args["id"]))

	switch action {
	case "", "view", "read", "list":
		return trackerView(name, when)
	case "add":
		newID, err := trackerAdd(name, when, text)
		if err != nil {
			return "[erreur] " + err.Error()
		}
		return fmt.Sprintf("[ok] point ajouté à « %s » (id %s).", name, newID)
	case "edit":
		if name == "" || id == "" {
			return "[erreur] name et id sont requis pour modifier un point"
		}
		if err := trackerEditEvent(slugifyIdent(name), id, when, text); err != nil {
			return "[erreur] " + err.Error()
		}
		return fmt.Sprintf("[ok] point %s de « %s » modifié.", id, name)
	case "delete":
		if name == "" {
			return "[erreur] name requis"
		}
		// Sans id, c'est le tracker ENTIER qu'on supprime : on l'exige explicite
		// pour qu'une suppression de point ratée n'efface pas tout l'historique.
		if id == "" {
			return "[erreur] id requis — supprimer un tracker entier se fait depuis l'interface, pas par l'outil"
		}
		if err := trackerDeleteEvent(slugifyIdent(name), id); err != nil {
			return "[erreur] " + err.Error()
		}
		return fmt.Sprintf("[ok] point %s de « %s » supprimé.", id, name)
	}
	return fmt.Sprintf("[erreur] action inconnue %q (view, add, edit, delete)", action)
}

// str extrait une chaîne d'un argument d'outil, tolérant au type (les modèles
// envoient volontiers un nombre là où le schéma dit « string »).
func str(v any) string {
	switch s := v.(type) {
	case string:
		return s
	case float64:
		return strings.TrimSuffix(fmt.Sprintf("%.0f", s), ".0")
	case nil:
		return ""
	}
