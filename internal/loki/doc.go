// Loki — cœur du binaire (package loki, appelé par cmd/loki). Go n'autorise
// pas de sous-dossiers dans un même package : les fichiers sont donc préfixés
// par domaine.
//
//	run.go        point d'entrée : dispatch des sous-commandes, LokiHome() et
...
//   - loki-engine (« loki serve ») exec llama-server ;
//   - loki-ui (« loki web ») sert l'UI locale, le tunnel du relais et
...
// `go generate ./internal/loki` (voir tools/assemble-ui).
package loki
