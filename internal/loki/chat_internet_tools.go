...
// web_open, web_read, web_grep) + la sous-commande `loki internet`.
package loki
...
// toolMaxOutput (8000) pour le shell. (La mémoire et MCP, eux, renvoient tout
// au modèle et laissent l'UI charger l'aperçu.) Sans ce
...
// ─── CLI : loki internet [on|off|status|engine|url|key] ────────────────────
...
func cmdInternet(args []string) error {
	...
	case "on":
		...
		if webEngine() == engineCrawl && crawl4aiURL() == "" {
			return fmt.Errorf("configure d'abord l'URL : loki internet url <url>  (ou bascule sur le moteur intégré : loki internet engine go)")
		}
	...
	case "url":
		if len(args) < 2 {
			return fmt.Errorf("usage: loki internet url <url>  (ex: http://localhost:11235)")
		}
	...
	case "key":
		if len(args) < 2 {
			return fmt.Errorf("usage: loki internet key <clé>  (vide pour l'enlever : loki internet key \"\")")
		}
	...
	case "engine":
		if len(args) < 2 {
			return fmt.Errorf("usage: loki internet engine <go|crawl4ai>")
		}
		...
		if e == engineGo {
			...
			fmt.Println(green("[ok]") + " moteur Crawl4AI — configure le serveur : loki internet url <url>")
		}
	case "", "status", "list":
		state := dim("off")
		if internetEnabled() {
		...
		u := crawl4aiURL()
		if u == "" {
			fmt.Printf("  serveur : %s — configure : loki internet url <url>\n", dim("(non configuré)"))
			return nil
		}
