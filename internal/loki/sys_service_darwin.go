...
// launchdLabel dérive le label launchd du service (ex. "com.loki.loki"), en
// conservant le préfixe hérité « com.loki. » tant que c'est LUI qui est
...
func launchdLabel(svc string) string {
	...
	if legacy := "com.loki." + svc; fileExists("/Library/LaunchDaemons/" + legacy + ".plist") {
		return legacy
	}
	return modern
}
...
// launchdLogPath : sortie standard/erreur du service, sous LOKI_HOME (accessible
...
func serviceAction(action string) error {
	...
	// via Loki.app sans jamais passer par `sudo loki install` : on gère alors le
	// service comme sous Windows, avec un `loki serve` détaché suivi par un fichier
...
// ---------------------------------------------------------------------------
// Mode utilisateur (sans launchd) — même approche que Windows : `loki serve` est
// lancé détaché, son PID va dans LOKI_HOME/<svc>.pid et sa sortie dans
// LOKI_HOME/<svc>.log. Setsid le détache de notre session pour qu'il survive à
// la fermeture de Loki.app, et permet de tuer tout le groupe (llama-server
...
func userSvcAction(action string) error {
	...
	case "enable", "disable":
		fmt.Printf("%s '%s' réclame un service système : lance %s pour installer le LaunchDaemon.\n",
			yellow("[info]"), action, bold("sudo loki install"))
		return nil
	}
	return fmt.Errorf("action inconnue: %s", action)
}
...
func userSvcStart() error {
	...
	// de config.env (MODEL=…gguf) se résolvent depuis LOKI_HOME, pas depuis « / »
