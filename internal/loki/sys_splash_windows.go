...
// sys_splash_windows.go — écran de démarrage « Lancement de Loki… ».
//
...
func createSplashWindow() uintptr {
	className, _ := syscall.UTF16PtrFromString("LokiSplash")
	...
	x := (int(cx) - splashW) / 2
	y := (int(cy) - splashH) / 2
	title, _ := syscall.UTF16PtrFromString("Loki")
...
func paintSplash(hdc uintptr) {
	...
	textX := ox + logo + 26
	drawText(hdc, "Loki", -30, 700, rect{textX, 40, splashW - 20, 82})
