...
//
// `loki llamacpp install`  installe un build neuf, détecte automatiquement
//                          l'accélérateur (CUDA / ROCm / Metal / CPU) et la
//                          compute capability du GPU, puis pointe BIN dessus.
// `loki llamacpp update`   met à jour le dépôt existant (git pull) et recompile
//                          avec la bonne config, sans intervention.
// `loki llamacpp status`   montre le commit courant, le backend détecté et le
...
// otherwise the default under $LOKI_HOME/backends/llama.cpp.
...
func llamacppInstall(args []string) error {
	...
	fmt.Printf("\nProchaines étapes :\n  1. renseigne MODEL : %s\n  2. démarre        : %s\n",
		bold("loki edit"), bold("loki restart"))
	return nil
}
...
func llamacppStatus(args []string) error {
	...
	fmt.Printf("  dépôt    : %s\n", repo)
	if !isDir(filepath.Join(repo, ".git")) {
		fmt.Printf("  %s pas encore installé — %s\n", yellow("état"), bold("loki llamacpp install"))
		return nil
	}
