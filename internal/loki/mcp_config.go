...
//
// loki peut se connecter à des serveurs MCP tiers pour enrichir la palette
// d'outils de l'IA (accès fichiers rapide type desktop-commander, bases de
// données, APIs métier…). Deux transports :
//
//   - stdio : loki lance un process local (command + args) et parle en
...
//   - http  : loki parle à un serveur MCP distant en Streamable HTTP (url +
...
// la machine où tourne loki — même niveau de confiance que l'outil `bash` du
