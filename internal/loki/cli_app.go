...
// cli_app.go — expérience « application » de Loki.
//
// Quand on double-clique sur le binaire (aucun argument, console fraîche), au
// lieu d'afficher l'aide dans une console qui se ferme, Loki démarre son UI web,
// l'ouvre dans le navigateur ET pose une icône dans la zone de notification
// Windows (voir sys_tray_windows.go) : on voit que Loki tourne et on le pilote
// (« Ouvrir Loki » / « Quitter »).
...
func cmdApp(args []string) error {
	...
	if err != nil {
		// Loki tourne déjà sur ce port : on ouvre juste l'UI sur l'instance
		// existante plutôt que d'échouer.
		fmt.Printf("Loki est déjà lancé — ouverture de %s\n", url)
		return openBrowser(url)
	}

	// UN SEUL process propriétaire de la conversation, comme le service loki-ui
	...
	sp := showSplash("Lancement d'Loki en cours…")
	waitServerReady(url)
	_ = openBrowser(url)
