...
// `loki install` simply provisions the data directory and a starter config; the
...
func cmdInstall(args []string) error {
	...
	// 2. Le binaire va dans LOKI_HOME\bin, ajouté au PATH utilisateur, pour que
	//    `loki` soit appelable depuis n'importe quel shell (pendant Windows du
	...
	if onPath {
		fmt.Printf("\n%s ouvre un NOUVEAU terminal pour que 'loki' soit reconnu (le PATH n'est lu qu'au démarrage du shell).\n", dim("[info]"))
	} else {
		fmt.Printf("\n%s pour exécuter 'loki' depuis n'importe où, ajoute son dossier au PATH.\n", dim("[info]"))
	}
	return nil
}

// installSelf copie l'exécutable en cours dans binDir sous le nom loki.exe et
...
// et Loki démarrait depuis le fichier téléchargé, sans jamais s'installer.
func installSelf(binDir string) (string, error) {
	...
	dst := filepath.Join(binDir, "loki.exe")
	...
	// replaceExe et non copyExe : loki.exe peut etre en cours d'execution
...
// l'alias « loki.exe » restait figé sur une version périmée dès qu'il tournait
...
func cmdUninstall(args []string) error {
	...
	// Pull LOKI_HOME\bin off the user PATH (best-effort).
	dir := binDir()
	...
	if removeShortcuts() {
		fmt.Printf("  %s raccourcis « Loki » supprimés\n", green("✓"))
	}

	if !keepData {
		lokiHome := LokiHome()
		if err := os.RemoveAll(lokiHome); err != nil {
			return fmt.Errorf("suppression de %s: %w", lokiHome, err)
		}
