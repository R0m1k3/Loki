...
//
// Avant : double-cliquer loki-windows-amd64.exe déclenchait une installation
// SILENCIEUSE (copie du binaire dans %ProgramData%\loki\bin, ajout au PATH) dont
...
//
//  1. Loki n'est pas installé. On installe, sans demander : rester à
...
//     copie installée.
//
//  2. Loki est déjà installé. Le fichier téléchargé se comporte en installeur de
...
func appFirstRun() bool {
	...
	if canonPath(exe) == canonPath(target) {
		...
		// retrouve plus Loki, et n'a aucune raison de relancer le fichier
	...
	if _, err := installSelf(filepath.Dir(target)); err != nil {
		// Échec le plus courant : pas les droits sur %ProgramData%. Loki reste
...
// runAsInstaller : Loki est déjà installé et on exécute une AUTRE copie (le
...
func runAsInstaller(target string) bool {
	...
	case cmp < 0:
		...
		if messageBox(
			"Une version plus récente d'Loki est déjà installée sur cet ordinateur.\n\n"+
				"    installée : "+installed+"\n"+
				"    ce fichier : "+Version+"\n\n"+
				"Rien ne sera remplacé.\n\n"+
				"Voulez-vous démarrer Loki (version "+installed+") ?\n"+
				"Répondre Non ferme simplement cette fenêtre.",
			"Loki", mbYesNo|mbIconQuestion) != idYes {
	...
	case cmp > 0 && len(running) > 0:
		// Mise à jour disponible, mais Loki tourne. Écrire le nouveau binaire
		...
		if messageBox(
			"Loki est déjà en cours d'exécution.\n\n"+
				"    version en cours : "+verLabel(installed)+"\n"+
				"    ce fichier : "+Version+" (plus récente)\n\n"+
				"Fermer Loki et le redémarrer pour appliquer la mise à jour ?\n\n"+
				"Répondre Non ouvre Loki dans sa version actuelle, sans rien mettre à jour.",
			"Mise à jour d'Loki", mbYesNo|mbIconQuestion) != idYes {
			ensureShortcuts(target)
		...
		if err := replaceInstalled(target); err != nil {
			messageBox("La mise à jour a échoué :\n\n"+err.Error()+"\n\nLoki va redémarrer dans sa version actuelle.",
				"Loki", mbIconInfo)
		}
		ensureShortcuts(target)
		return launch(target)
...
// (même mécanique que replaceBinary pour `loki update`).
...
// le CHEMIN, pas le nom : tuer par nom d'image (« loki.exe ») emporterait aussi
...
// posées par goversioninfo, cf cmd/loki/versioninfo.json), comme le fait
// l'onglet « Détails » des propriétés Windows.
//
// ⚠️ NE PAS remplacer par un `loki version` exécuté : le binaire est compilé en
...
// ensureShortcuts garantit qu'un raccourci « Loki » existe dans le menu
...
foreach ($d in @($progs, [Environment]::GetFolderPath('Desktop'))) {
  ...
  $lnk=Join-Path $d 'Loki.lnk'
  # Un raccourci existant est REPOINTÉ s'il vise autre chose que le binaire
  # canonique — typiquement l'ancien « loki.exe », qui n'est plus qu'un alias.
  ...
  try {
    ...
    $s.TargetPath=$t
    $s.WorkingDirectory=(Split-Path $t)
    $s.Description='Loki, votre IA locale'
    # Minimisé (7) : le binaire étant en sous-système console, Windows lui alloue
    # une console au lancement. Loki la referme aussitôt, mais demander un
...
Write-Output ($done -join ';')`, psQuote(target))
	...
	if err != nil || strings.TrimSpace(string(out)) == "" {
		return "(raccourcis non créés — lancez Loki depuis " + target + ")"
	}
	return "Raccourci « Loki » ajouté au menu Démarrer et au Bureau."
}
