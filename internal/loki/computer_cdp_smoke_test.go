...
// navigateur installé → ne tourne QUE si LOKI_CU_SMOKE=1.
func TestCDPSmoke(t *testing.T) {
	if os.Getenv("LOKI_CU_SMOKE") != "1" {
		t.Skip("smoke test désactivé (LOKI_CU_SMOKE=1 pour l'activer)")
	}
	if chromePath() == "" {
...
func TestCDPTypeReplacesField(t *testing.T) {
	...
	if _, err := s.snapshot(); err != nil { // pose les data-loki-ref
		t.Fatalf("snapshot: %v", err)
	}
	// Retrouve le numéro du 2e champ (celui à remplacer).
	val, err := s.eval(`document.querySelector('#b').getAttribute('data-loki-ref')`)
	if err != nil {
...
// EFFACER les anciens data-loki-ref, sinon deux éléments partagent un numéro et
...
func TestCDPRefUniqueness(t *testing.T) {
	...
	if _, err := s.eval(`document.querySelectorAll('a')[2].setAttribute('data-loki-ref','1');'ok'`); err != nil {
	...
	dup, _ := s.eval(`document.querySelectorAll('[data-loki-ref="1"]').length`)
	if strings.TrimSpace(string(dup)) != "1" {
		t.Errorf("data-loki-ref=\"1\" devrait être unique, trouvé %s occurrence(s)", dup)
	}
}
...
func TestCDPTypeSelectsOption(t *testing.T) {
	...
	ref, _ := s.eval(`+document.querySelector('#civ').getAttribute('data-loki-ref')`)
...
// les data-loki-ref du dernier snapshot (sinon un browser_click ensuite échoue).
func TestCDPFindKeepsRefs(t *testing.T) {
	...
	before, _ := s.eval(`document.querySelectorAll('a')[2].getAttribute('data-loki-ref')`)
	// Recherche d'un AUTRE élément : ne doit pas toucher au ref de « Connexion ».
	if _, err := s.find("Alpha"); err != nil {
		t.Fatalf("find: %v", err)
	}
	after, _ := s.eval(`document.querySelectorAll('a')[2].getAttribute('data-loki-ref')`)
...
func TestCDPNavigateError(t *testing.T) {
	...
	defer cdpShutdown()
	err = s.navigate("https://nonexistent-host-loki-test-99999.invalid")
	if err == nil {
