...
// WorkingDirectory, LOKI_HOME, StandardOutPath, StandardErrorPath.
...
<dict>
  ...
  <dict><key>LOKI_HOME</key><string>%s</string></dict>
...
func installServices(targetUser, lokiHome string) error {
	...
	for name, arg := range unitArgs() {
		logPath := filepath.Join(lokiHome, name+".log")
		plist := fmt.Sprintf(launchdPlistTemplate,
			launchdLabel(name), exe, arg, targetUser, lokiHome, lokiHome, logPath, logPath)
