...
// ouvrir, lire et copier pour archive. La 0.8 l'a rangé dans loki.db (bbolt) :
...
// Loki. Le fil de discussion et les raisonnements sont toujours là, mais plus
...
type exportPayload struct {
	Version    string     `json:"loki_version"`
...
func (c *Conversation) ExportMarkdown(o exportOpts) string {
	...
	fmt.Fprintf(&b, "# Conversation Loki\n\nExportée le %s par Loki %s.\n",
	...
	// été écrit). Sert à ne poser l'en-tête « Loki » qu'une fois par tour, même
...
func exportFilename(ext string) string {
	return "loki-conversation-" + time.Now().Format("2006-01-02-1504") + "." + ext
}
...
//
//	loki export                  → loki-conversation-<date>.md dans le dossier courant
//	loki export --json           → idem en JSON
//	loki export mon-fil.md       → nom de fichier imposé (l'extension choisit le format)
//	loki export -                → sur la sortie standard, pour enchaîner un tube
//
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			if strings.HasPrefix(a, "-") && a != "-" {
				return fmt.Errorf("option inconnue : %s (voir « loki help »)", a)
			}
			out = a
		}
	}
	// Une extension explicite l'emporte sur le drapeau : `loki export fil.json`
