...
func cmdWeb(args []string) error {
	...
	if readLinkToken() != "" {
		...
		if fp := e2eFingerprint(); fp != "" {
			fmt.Printf("%s empreinte E2E : %s   (code d'appairage : loki link code)\n", green("[e2e]"), bold(fp))
		}
...
// réutilisé par `loki link`, qui sert ce même mux à travers le tunnel sans
...
func newWebMux() *http.ServeMux {
	...
	// Alias rétro-compat : l'ancien portail ajean.link (dépôt loki-relay) pilote
	...
	api("/api/chat/files/zip", handleChatFilesZip)     // toute la discussion en une archive .zip
...
func processName(pid int) string {
	if runtime.GOOS == "windows" {
		// tasklist CSV : "loki.exe","1234","Console","1","12 345 K"
