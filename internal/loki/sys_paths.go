...
// et on l'expose à la fois en ligne de commande (`loki where`) et dans l'UI.

type lokiPaths struct {
	Home      string `json:"home"`      // LOKI_HOME : racine des données
	Database  string `json:"database"`  // loki.db : config, préférences, conversation, clés
...
// Il diffère par plateforme : sous Unix les installateurs posent /usr/local/bin/loki
// (c'est ce que référencent les unités systemd et le plist launchd) ; sous Windows,
// faute d'équivalent, on utilise LOKI_HOME\bin ajouté au PATH utilisateur.
func installedExePath() string {
	if runtime.GOOS == "windows" {
		return filepath.Join(binDir(), "loki.exe")
	}
	return "/usr/local/bin/loki"
}

func currentPaths() lokiPaths {
	exe, _ := os.Executable()
...
func cmdWhere(args []string) error {
	p := currentPaths()
	fmt.Printf("Emplacements Loki\n\n")
	for _, row := range [][2]string{
		{"données (LOKI_HOME)", p.Home},
		{"base", p.Database},
