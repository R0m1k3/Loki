...
func toolTaskList(args map[string]any) string {
	...
	if id := strings.TrimSpace(taskStrArg(args["id"])); id != "" {
...
func toolTaskCreate(args map[string]any) string {
	name := strings.TrimSpace(taskStrArg(args["name"]))
	prompt := strings.TrimSpace(taskStrArg(args["prompt"]))
	script := strings.TrimSpace(taskStrArg(args["script"]))
	schedule := strings.TrimSpace(taskStrArg(args["schedule"]))
	if name == "" || schedule == "" {
...
func toolTaskUpdate(args map[string]any) string {
	id := strings.TrimSpace(taskStrArg(args["id"]))
	if id == "" {
		return "[erreur] id manquant"
	}
	t, ok := getTask(id)
	if !ok || taskProjectOf(t) != activeProjectSlug() {
		return "[erreur] tâche introuvable : " + id
	}
