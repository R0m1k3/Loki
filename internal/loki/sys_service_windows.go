...
// `loki serve` that this process launches detached. We track it with a PID file
// and stream its output to a log file under LOKI_HOME. This needs no admin
...
func svcStart() error {
	...
	cmd.Stdout = logf
	cmd.Stderr = logf
	cmd.Dir = LokiHome() // les chemins relatifs de config.env se résolvent depuis LOKI_HOME
	// createNoWindow + HideWindow : le service enfant (`loki serve`) ne doit JAMAIS
	// faire clignoter de console noire quand Loki est lancé en mode app (double-clic).
