...
// que loki traduisait par « droits insuffisants, relance en administrateur » :
...
func TestRenameAsideNeBloquePasSurUnAncienEcartement(t *testing.T) {
	dir := t.TempDir()
	exe := filepath.Join(dir, "loki.exe")
	...
	if !strings.HasPrefix(filepath.Base(aside), "loki.exe.old-") {
