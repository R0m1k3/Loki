...
// Les .gguf vivent dans $LOKI_HOME/models, mais un modèle de 40 Go tient
...
// peut aussi venir de $LOKI_MODEL_DIRS.

// modelDirsListSep sépare les dossiers dans $LOKI_MODEL_DIRS (':' sous Unix,
...
func extraModelDirs() []string {
	...
	if v := os.Getenv("LOKI_MODEL_DIRS"); v != "" {
...
// relu par un loki Linux, où filepath.Base ne coupe pas sur '\'.
...
// fichier est cherché dans chaque dossier, LOKI_HOME en premier.
...
// handleModelDirs : GET liste les dossiers de modèles (LOKI_HOME + ajoutés),
