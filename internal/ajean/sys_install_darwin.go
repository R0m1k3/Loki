...
// WorkingDirectory, AJEAN_HOME, AJEAN_HOME, StandardOutPath, StandardErrorPath.
...
<dict>
  ...
  <dict><key>AJEAN_HOME</key><string>%s</string><key>AJEAN_HOME</key><string>%s</string></dict>
...
func cmdInstall(args []string) error {
	...
	// Migration de l'agencement ajean -> ajean AVANT de résoudre les chemins :
	...
	// 3 bis. Alias hérité : `ajean` doit rester tapable (voir sys_paths.go).
	...
	// 5. LaunchDaemon plist (le log va sous AJEAN_HOME, accessible au user cible)
	...
	// 6. chown AJEAN_HOME au user cible
	chown(ajeanHome, u)
...
func cmdUninstall(args []string) error {
	...
	// On ne supprime jamais AJEAN_HOME automatiquement (modèles, configs).
